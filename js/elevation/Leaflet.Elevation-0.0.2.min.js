...
L.Control.Elevation = L.Control.extend({
    ...
    onAdd: function(a) {
        ...
        var d = this._x = d3.scale.linear().range([0, b.width]),
            e = this._y = d3.scale.linear().range([b.height, 0]),
    ...
    _dragEndHandler: function() {
        ...
        var a = this._findItemForX(this._dragStartCoords[0]),
            ...
            c = L.latLngBounds(a.latlng, a.latlng);
    ...
    _appendXaxis: function(a) {
        a.attr("class", "x axis").attr("transform", "translate(0," + this.options.height + ")").call(d3.svg.axis().scale(this._x).ticks(this.options.xTicks).orient("bottom")).append("text").attr("x", this.options.width + 22).attr("y", 18).style("text-anchor", "end").text("km")
    },
    ...
    _mousemoveHandler: function() {
        if (this._data && 0 !== this._data.length) {
            ...
            var c = d3.bisector(function(a) {
                ...
                d = this._x.invert(a[0]),
                e = c(this._data, d),
                f = this._data[e].altitude,
                g = this._data[e].dist,
                h = this._data[e].latlng,
                i = b.hoverNumber.formatter(f, b.hoverNumber.decimalsY),
                j = b.hoverNumber.formatter(g, b.hoverNumber.decimalsX);
            ...
            if (b.useHeightIndicator) {
                ...
                var n = this.options.height / this._maxElevation * f,
                    o = k.y - n;
    ...
    _addGeoJSONData: function(a) {
        if (a) {
            ...
            for (var b = this._data || [], c = this._dist || 0, d = this._maxElevation || 0, e = 0; e < a.length; e++) {             
                ...
                var valeur_actuelle=parseInt(a[e][2]);
				var valeur_origine=parseInt(a[0][2]);
                
				(e>0) ? valeur_ant=parseInt(a[e-1][2]) : valeur_ant=valeur_origine;
                    
                ((valeur_actuelle-valeur_ant)>0) ? dplus += valeur_actuelle-valeur_ant : dmoins += (Math.abs(valeur_actuelle-valeur_ant));
				...
				dplus = Math.round(parseInt(dplus));
				dmoins = Math.round(parseInt(dmoins));
            }
            $('<span id="titre">' + a.properties.name + '</span>').appendTo('.leaflet-control.elevation');
			$('<span class="deniv"><i class="fas fa-arrow-up"></i> '+ dplus.toString() + ' m &nbsp; &nbsp;    <i class="fas fa-arrow-down"></i> ' + dmoins.toString() + ' m</span>').appendTo('.leaflet-control.elevation');

            if(a.properties.link){
                $('<span id="lien-carte" class="dl-link"><a target="_blank" href="' + a.properties.link + '">Voir plus</a></span>').appendTo('.leaflet-control.elevation');
            }
